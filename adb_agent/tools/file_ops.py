...
def list_files(remote_path: str = "/sdcard/") -> dict:
    """List files and directories at a path on the Android device.

    Args:
        remote_path: Directory path on the device to list. Default '/sdcard/'.

    Returns:
        dict with status and file listing.
    """
    result = subprocess.run(
    ...
    if result.returncode != 0:
        return {"status": "error", "error_message": result.stderr}
    return {"status": "success", "files": result.stdout.strip()}


def delete_file(remote_path: str) -> dict:
    """Delete a file on the Android device. Only allows deletion under /sdcard/ for safety.

    Args:
        remote_path: Path to the file to delete on the device. Must be under /sdcard/.

    Returns:
        dict with status and message.
    """
    # Safety check: only allow deletion under /sdcard/
    if not remote_path.startswith("/sdcard/"):
        return {"status": "error", "error_message": "For safety, only files under /sdcard/ can be deleted."}
    ...
    if result.returncode != 0:
        return {"status": "error", "error_message": result.stderr}
    return {"status": "success", "message": f"Deleted {remote_path}"}
